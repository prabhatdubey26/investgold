...
(function ($) {
  ...
  var loadMyCartEvent = function (targetSelector) {
    ...
    let products = JSON.stringify(ProductManager.getAllProducts(),true);
    ...
    if (!$("#" + idCartModal).length) {
      $('body').append(
        ...
        '<span class="close" onclick="closeModal()" style="margin-left: 350px;">×</span>' +
        '</div>' +
        ...
        '<button type="submit" class="btn btn-primary ' + classCheckoutCart + '">Checkout</button>' +
        '</form>' +
        '</div>' +
        '</div>' +
        '</div>' +
...
// Function to update the checkout form with current cart data
var updateCheckoutForm = function() {
  let products = JSON.stringify(ProductManager.getAllProducts());
  let totalPrice = ProductManager.getTotalPrice();
  
  $('#checkoutForm input[name="products"]').val(products);
  $('#checkoutForm input[name="total_price"]').val(totalPrice);
};
